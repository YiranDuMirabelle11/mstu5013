...
<%--
    ...
    //Data reference
    var msgByChannelRef = database.ref('msgByChannel');
    var ChannelsRef = database.ref('Channels');
		...
		//
		// msgByChannelRef.on('value', function(snapshot){
   ...
   ChannelsRef.on('value', function(snapshot) {
		 var ChannelsData = snapshot.val();
		 that.Channels = [];

		 for (key in ChannelsData) {
 				that.Channels.push(ChannelsData[key]);
 			}
